...
emDia dataAtual (codLivro, codAluno, dataEmprest, dataDevo, status) =
  procede dataAtual dataDevo

atrasados :: Emprestimos -> Data -> Emprestimos
atrasados listaEmprestimos dataAtual = [x | x <- listaEmprestimos, not (emDia dataAtual x)]
...
uniaoNRec lista1 lista2 = [x | x <- lista1] ++ [y | y <- lista2, not (pertence y lista1)]
